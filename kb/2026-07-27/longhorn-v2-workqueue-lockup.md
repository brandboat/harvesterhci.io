...
Longhorn added the `data-engine-cpu-isolation-enabled` setting to mitigate this issue. When enabled for the V2 Data Engine, the Longhorn V2 instance-manager:
...
This setting steers IRQs and unbound workqueues away from SPDK CPUs. It does not move CPU-bound or per-CPU kernel workers such as `kworker/0:*`, which are tied to a specific CPU by the kernel.

Use this setting when the Longhorn version bundled with Harvester includes it. As of July 27, 2026, Harvester UI exposure for this setting is planned for v1.10.0. If the Longhorn setting exists but Harvester does not expose it in the UI, you can still configure it through the Longhorn setting resource. For Harvester versions earlier than v1.9.0, the bundled Longhorn version does not include the setting, so use the manual workaround in the next section.
...
Use this workaround for Harvester versions earlier than v1.9.0, where the bundled Longhorn version does not include `data-engine-cpu-isolation-enabled`. For later versions, first check whether the Longhorn setting exists and prefer the setting-based mitigation when possible.
